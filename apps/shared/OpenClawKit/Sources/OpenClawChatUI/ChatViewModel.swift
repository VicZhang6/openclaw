...
public final class OpenClawChatViewModel {
    ...
    private func loadSlashCommands(force: Bool) async {
        ...
        do {
            self.slashCommands = try await self.transport.listCommands()
    ...
    private func validateSlashCommandDraftForSend(trimmed: String) async -> Bool {
        ...
        if self.hasLoadedSlashCommands {
            guard Self.isKnownSlashCommandText(trimmed, commands: self.slashCommands) else {
                self.errorText = "Unknown command: /\(slashName)"
                return false
            }
            guard self.attachments.isEmpty else {
